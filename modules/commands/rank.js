...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
	...
	if (args.length == 0) {
		...
		return api.sendMessage({body: `👀${name}\n🏆Top ${rank}\n📝 ${Date.now() - timeStart} Tin nhắn ✅`, attachment: fs.createReadStream(pathRankCard, {'highWaterMark': 128 * 1024}) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
	}
	if (mention.length == 1) {
		...
		return api.sendMessage({body: `👀${name}\n🏆Top ${rank}\n📝 ${Date.now() - timeStart} Tin nhắn ✅`, attachment: fs.createReadStream(pathRankCard) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
	}
	if (mention.length > 1) {
